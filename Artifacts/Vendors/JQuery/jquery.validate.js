    ...
    $.extend($.fn,
        {
            ...
            rules: function(command, argument) {
                ...
                const isContentEditable = typeof this.attr("contenteditable") !== "undefined" &&
                    this.attr("contenteditable") !== "false";
                var settings, staticRules, existingRules, data, param, filtered;
    ...
    $.extend($.validator,
        {
            ...
            prototype: {
                ...
                elementValue: function(element) {
                    ...
                    const isContentEditable = typeof $element.attr("contenteditable") !== "undefined" &&
                    ...
                    for (method in rules) {
                        ...
                        try {
                            ...
                            if (this.settings.debug && window.console) {
                                console.log(
                                    `Exception occurred when checking element ${element.id}, check the '${rule.method
                                    }' method.`,
                                    e);
                            }
                            if (e instanceof TypeError) {
                                e.message += `.  Exception occurred when checking element ${element.id}, check the '${
                                    rule.method}' method.`;
                            }
                ...
                customDataMessage: function(element, method) {
                    return $(element)
                        .data(`msg${method.charAt(0).toUpperCase()}${method.substring(1).toLowerCase()}`) ||
                        $(element).data("msg");
                },
...
// Use a prefilter if available (1.5+)
    if ($.ajaxPrefilter) {
        ...
        $.ajax = function(settings) {
            const mode = ("mode" in settings ? settings : $.ajaxSettings).mode;
            const port = ("port" in settings ? settings : $.ajaxSettings).port;
            if (mode === "abort") {
                if (pendingRequests[port]) {
                    pendingRequests[port].abort();
                }
